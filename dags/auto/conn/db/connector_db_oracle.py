...
from dags.auto.conn.db.connector_db_abc import ConnectorDbAbc


class ConnectorDbOracle(ConnectorDbAbc):
    def __init__(self, object_prop,
        ...
        dsn = f"oracle+oracledb://{user}:{password}@{host}:{port}/?service_name={database}"
        ...
        self.conn = oracledb.connect(
            user=user,
            password=password,
            dsn=f"{host}:{port}/?service_name={database}"
        )
    ...
    def write_table(self, schema, table, dataframe, has_a_sequence, sequence_name, identity_column,
                    insert_template=None, page_size=100):
