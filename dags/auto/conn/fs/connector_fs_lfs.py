...
class ConnectorFsLfs(ConnectorFsAbc):

    def __init__(self, object_prop: dict = None):
        super().__init__()
    ...
    def write_txt(self, path, data, mode="w"):
        with open(path, f"{mode}t") as txt_file:  # pylint: disable=W1514
            txt_file.write(data)

    def write_bin(self, path, data, mode="w"):
        with open(path, f"{mode}b") as bin_file:
            bin_file.write(data)
