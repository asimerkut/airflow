...
from dags.auto.conn.fs.connector_fs_abc import ConnectorFsAbc

lfs_data_path = env.PRM_LFS_PATH + "/data"
lfs_tmp_path = env.PRM_LFS_PATH + "/tmp"


class ConnectorFsRfs(ConnectorFsAbc):
    ...
    def write_json(self, path, dataframe, orient="records", **kwargs):
        ...
        if isinstance(dataframe, pd.DataFrame):
            ...
            with open(local_path, "wt") as file:  # pylint: disable=W1514
                file.write(out)
        else:
            raise Exception(f"unsupported :{type(dataframe)}")
    ...
    def write(self, path, data, mode="wt", a_sftp=None):
    ...
    def write_bin(self, path, data, mode="w", a_sftp=None):
        sftp = self.__create_temp_sftp(a_sftp)
        # mod icin alternatif  (path, mode + "b")
        with sftp.open(path, f"{mode}b") as bin_file:
            bin_file.write(data)
