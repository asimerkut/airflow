...
import dags.auto.util.sp_to_pd as b
from dags.auto.util import static_util
from dags.auto.util.sp_converter import spark_to_pandas_filter


def join_dfs(cmp, df1, df2, how, left_on, right_on):
    df_join = pd.DataFrame()
    if how != "left_anti":
        df_join = pd.merge(df1, df2, how=how, left_on=left_on, right_on=right_on,suffixes=("__t1", "__t2"))
    else:
        anti_join = df1.merge(df2, left_on=left_on, right_on=right_on, how='left', indicator=True)
        df_join = anti_join[anti_join['_merge'] == 'left_only']
        df_join.drop(["_merge"], axis=1, inplace=True)
    # for c in list(df_join.columns):
    #    if c.endswith("_€"):
    #        df_join.rename(columns = {c:c.split("_€")[0]}, inplace = True)
    return df_join


def column_filter_dfs(cmp, columns_array, df):
    df_select = df[columns_array]
    return df_select


def column_rename_dfs(cmp, df, new_column_name, old_column_name):
...
def constant_value_dfs(cmp, column_name, column_value, column_type, df):
...
def data_head_dfs(cmp, df, size):
    return df.iloc[:size, ]


def normalized_dfs(cmp, df, normalizer, range, columns):
    if normalizer == 'Min-Max Normalization':
        ...
        raise Exception('Bilinmeyen normalizer: ' + normalizer)
...
def denormalized_dfs(cmp, df, model, denormalizer, columns):
    if (denormalizer == 'Min-Max Normalization') | (denormalizer == 'Z-Score Normalization'):
        df[columns] = model.inverse_transform(df[columns])
    else:
        raise Exception('Bilinmeyen denormalizer ' + denormalizer)
    if isinstance(df, np.ndarray):
        df = pd.DataFrame(df)
    return df


def distinct_dfs(cmp, df=None, subset=None):
    df_distinct = df.drop_duplicates(subset=subset, keep="first")
    return df_distinct


def groupby_dfs(cmp, agg_props, column_props, by, df, dropna):
    agg_params = {}
...
def listagg_dfs(cmp, column_props, by, df, dropna):
...
def window_function_dfs(cmp, df, column_name, partition_by, order_by, operation, shift_value):
    ...
    # Sıralamayı yapmak için order_by sütununu kullanarak DataFrame'i sırala
    # df_sorted = df.sort_values(by=order_by)
    # Grup bazında işlem yapmak için group_by sütununa göre gruplama yap
...
def standardization_dfs(cmp, df):
...
def row_filter_dfs(cmp, df, query_expr, df_id="df", inverse=False):
...
def sorter_dfs(cmp, ascending, by, df):
    kind = "quicksort"
    na_position = "first"
...
def string_replacer_dfs(cmp, append, append_column_name, df, pattern, replace_text, target_column_name):
    if append == True:
...
def union_dfs(cmp, df1, df2):
    if len(df1.columns) != len(df2.columns):
        raise Exception(f"kolon sayıları eşit değil! {df1.columns} != {df2.columns}")

    df2_copy = df2.to_dictionary()
    df2_copy = df2_copy.rename(columns=dict(zip(df2_copy.columns, df1.columns)))

    df_union = pd.concat([df1, df2_copy], axis=0, ignore_index=True)
    return df_union
...
def data_splitter_dfs(cmp, how, df, stratified_sampling_column, absolute_or_relative, split_value):
...
def data_sampler_dfs(cmp, how, df, stratified_sampling_column, absolute_or_relative, proportion):
    df_first, df_second = data_splitter_dfs(cmp=cmp, how=how, df=df,
...
def denormalizer(cmp, df, model, denormalizer, columns):
    df = denormalized_dfs(df, model, denormalizer, columns)
    return df


def equal_size_data_sampler(cmp, df, column_name):
    # En az tekrar eden sınıfın sayısını bul
    min_count = df[column_name].value_counts().min()
    # Her bir sınıftan en az tekrar eden sınıf kadar örnek al
