...
import dags.auto.lib.ai.impl_ai as LibAi
import dags.auto.lib.ai.impl_ai_association_fpgrowth as LibAiAssociationFpgrowth
import dags.auto.lib.ai.impl_ai_decision_tree as LibAiDecisionTree
import dags.auto.lib.ai.impl_ai_isolation_forest as LibAiIsolationForest
import dags.auto.lib.ai.impl_ai_light_gbm as LibAiLightGbm
import dags.auto.lib.ai.impl_ai_linear_regression as LibAiLinearRegression
import dags.auto.lib.ai.impl_ai_logistic_regression as LibAiLogisticRegression
import dags.auto.lib.ai.impl_ai_random_forest as LibAiRandomForest
import dags.auto.lib.ai.impl_ai_tabnet_deep as LibAiTabnetDeep
import dags.auto.lib.ai.impl_ai_xg_boost as LibAiXgBoost
import dags.auto.env as env
from dags.auto.util.enum_util import LibEnum, KeyEnum


def lib_class():
    return LibEnum.LibAi
...
def string_indexer(cmp, object_prop, object_vars, df):
    ...
    out_df, var_pdf = LibAi.string_indexer(cmp=cmp, df=df, columns_array=columns_array, remove_columns=remove_columns)
    return object_vars, out_df, var_pdf


def association_fpgrowth(cmp, object_prop, object_vars, df):
    rules, results = LibAi.association_fpgrowth(cmp=cmp, df=df, object_prop=object_prop)
    return object_vars, rules, results


def correlation_matrix(cmp, object_prop, object_vars, df):
    corr_matrix = LibAi.correlation_matrix(cmp=cmp, df=df)
...
def k_means(cmp, object_prop, object_vars, df):
    ...
    df, kmeans = LibAi.k_means(cmp=cmp, cluster_num=cluster_num, df=df, max_iter=max_iter, n_init=n_init)
...
def roc_curve(cmp, object_prop, object_vars, df):
    ...
    return object_vars, buf_svg.read(), buf_png.read()


def score(cmp, object_prop, object_vars, df):
    actual_column = object_prop["actual_column"]
    pred_column = object_prop["pred_column"]
    df_conf_mat, df_metrics = LibAi.score(cmp=cmp, df=df, actual_column=actual_column, pred_column=pred_column)
    try:
        print("------------------------------------------------")
...
def z_score(cmp, object_prop, object_vars, df):
    df_zscore = LibAi.z_score(cmp=cmp, df=df)
    return object_vars, df_zscore


def ai_model_save(cmp, object_prop, object_vars, model):
...
def ai_model_load(cmp, object_prop, object_vars):
    model_kc_arr = object_prop["model_kc"].split(":")
    model_item = auto.get_system_db().get_item_value(model_kc_arr[0], model_kc_arr[1])
    model_path = env.PRM_MODEL_PATH + "/model_" + str(model_item["id"]).replace("-", "") + ".pkl"
