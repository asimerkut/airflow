...
from dags.auto.util.enum_util import LibEnum


def lib_class():
    return LibEnum.LibIoFilesys


def filesys_copy(cmp, object_prop, object_vars, filesys_connector, df):
...
def filesys_create_folder(cmp, object_prop, object_vars, filesys_connector, df):
...
def filesys_create_temp_folder(cmp, object_prop, object_vars, filesys_connector, df):
    parent = object_prop["parent"]
...
def filesys_delete_filefolder(cmp, object_prop, object_vars, filesys_connector, df):
...
def filesys_file_exists(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
    fs_data = pd.DataFrame(columns=["path"], data=[[filesys_connector.exists(path)]])
    return object_vars, df, fs_data


def filesys_file_info(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
    if not filesys_connector.exists(path):
        raise Exception("dosya/dizin yok! :" + path)
...
def filesys_get_hashcode(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_get_permissions(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_list_files(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_move(cmp, object_prop, object_vars, filesys_connector, df):
    source = object_prop["source"]
    dest = object_prop["dest"]
    overwrite = object_prop["overwrite"]
...
def filesys_read_binary(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_csv(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_excel(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_json(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_parquet(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_png(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]

    if not filesys_connector.exists(path):
        raise Exception("dosya veya dizin yok! :" + path)

    png_binary = filesys_connector.read_binary(path)
    return object_vars, png_binary


def filesys_read_svg(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_txt(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_read_xml(cmp, object_prop, object_vars, filesys_connector):
    path = object_prop["path"]
...
def filesys_set_permissions(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_unzip_file(cmp, object_prop, object_vars, filesys_connector, df):
    source = object_prop["source"]
    dest = object_prop["dest"]
    overwrite = object_prop["overwrite"]
...
def filesys_write_bin(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_csv(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_excel(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_json(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_parquet(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_png(cmp, object_prop, object_vars, filesys_connector, data):
    path = object_prop["path"]
...
def filesys_write_svg(cmp, object_prop, object_vars, filesys_connector, data):
    path = object_prop["path"]
...
def filesys_write_txt(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
...
def filesys_write_xml(cmp, object_prop, object_vars, filesys_connector, df):
    path = object_prop["path"]
    columns = object_prop["column_names"]
...
def filesys_zip_file(cmp, object_prop, object_vars, filesys_connector, df):
    source = object_prop["source"]
    dest = object_prop["dest"]
    overwrite = object_prop["overwrite"]
