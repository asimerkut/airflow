...
def lib_class():
    return LibEnum.LibStat


def analysis_of_variance_test(cmp, object_prop, object_vars, df):
    params = []
...
def anderson_darling_test(cmp, object_prop, object_vars, df):
...
def augmented_dickey_fuller_test(cmp, object_prop, object_vars, df):
    df = df[object_prop["column_name"]]
...
def chi_squared_test(cmp, object_prop, object_vars, df):
...
def d_agostino_k_squared_test(cmp, object_prop, object_vars, df):
    df = df[object_prop["column_name"]]
...
def friedman_test(cmp, df, object_prop):
    params = []
    for c in object_prop["column_names"]:
        params.append(df[c])
...
def kendalls_rank_correlation_test(cmp, object_prop, object_vars, df):
...
def kruskal_wallis_h_test(cmp, object_prop, object_vars, df):
    params = []
    for c in object_prop["column_names"]:
        params.append(df[c])
...
def kwiatkowski_phillips_schmidt_shin_test(cmp, object_prop, object_vars, df):
    df = df[object_prop["column_name"]]
...
def mann_whitney_u_test(cmp, object_prop, object_vars, df):
...
def paired_students_t_test(cmp, object_prop, object_vars, df):
...
def pearson_correlation_coefficient_test(cmp, object_prop, object_vars, df):
    df1 = df[object_prop["column_name_1"]]
    df2 = df[object_prop["column_name_2"]]
...
def repeated_measures_anova_test(cmp, object_prop, object_vars, df):
...
def shapiro_wilk_test(cmp, object_prop, object_vars, df):
    df = df[object_prop["column_names"]]
...
def spearmans_rank_correlation_test(cmp, object_prop, object_vars, df):
    df1 = df[object_prop["column_name_1"]]
    df2 = df[object_prop["column_name_2"]]
...
def students_t_test(cmp, object_prop, object_vars, df):
...
def wilcoxon_signed_rank_test(cmp, object_prop, object_vars, df):
